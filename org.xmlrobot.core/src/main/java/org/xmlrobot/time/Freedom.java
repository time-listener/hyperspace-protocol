...
public abstract class Freedom 
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5778447688615775982L;
	 ...
	 */
	protected Freedom(Class<? extends K> type, K parent, V child) {
	 ...
	 * @param parent the parent
	 */
    protected Freedom(Class<? extends K> type, V value, K parent, V child) {
	 ...
	 */
	protected Freedom(Class<? extends K> type, K root, V stem, Parity gen) {
	 ...
	 * @param root the parent
	 */
    protected Freedom(Class<? extends K> type, V value, K root, V stem, Parity gen) {
	...
	/* (non-Javadoc)
	 * @see org.xmlrobot.genesis.PastCallable#removeKey(org.xmlrobot.genesis.TimeListener, org.xmlrobot.horizon.Tachyon)
	 */
	@Override
	public synchronized void redeemKey(MassListener sender, Tachyon<K,V> event) {
		// call recursive recurrence
		getParent().removeKey(sender, event);
	}
	/* (non-Javadoc)
	 * @see org.xmlrobot.genesis.RunnableFuture#removeValue(org.xmlrobot.genesis.TimeListener, org.xmlrobot.horizon.Tachyon)
	 */
	@Override
	public synchronized void redeemValue(MassListener sender, Tachyon<V,K> event) {
		// call recurrent recursion
		getParent().removeValue(sender, event);
	}
    /* (non-Javadoc)
     * @see org.xmlrobot.genesis.Recursion#redeemKey(org.xmlrobot.TimeListener, org.xmlrobot.horizon.Tachyon)
     */
    @Override
    public void liberateKey(MassListener sender, Tachyon<K,V> event) {
		// call recursive recursion
		getChild().removeValue(sender, event);
    }
    /* (non-Javadoc)
     * @see org.xmlrobot.genesis.Recursion#redeemValue(org.xmlrobot.TimeListener, org.xmlrobot.horizon.Tachyon)
     */
    @Override
    public void liberateValue(MassListener sender, Tachyon<V,K> event) {
		// call recursive recursion
		getChild().removeKey(sender, event);
    }
	/* (non-Javadoc)
	 * @see org.xmlrobot.genesis.Past#releaseParent(java.lang.Object)
	 */
	@Override
	public synchronized void releaseKey(MassListener sender, Tachyon<K,V> event) {
	//  get recursive concurrence
		get().removeValue(sender, event); 
	}
	/* (non-Javadoc)
	 * @see org.xmlrobot.genesis.Future#releaseChild(java.lang.Object)
	 */
	@Override
	public synchronized void releaseValue(MassListener sender, Tachyon<V,K> event) {
		//  get concurrence recursion
		get().removeKey(sender, event);
	}
}